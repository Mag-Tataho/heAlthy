...
const defaultSchedules = {
  ...
  mealReminders:    { time: '12:00', label: '🍽️ Meal Reminder',    intervalHours: 0,  startTime: '12:00', endTime: '12:00' },
...
export default function Settings() {
      ...
      const fireIfInRange = () => {
        ...
        if (hour >= startH && hour < endH) {
          new Notification('heAlthy Reminder 🌿', {
            body: `${label} — Don't forget to drink water! 💧`,
            icon: '/favicon.ico',
          });
        }
      };
  ...
  const reminderItems = [
    { key: 'waterReminder',    label: '💧 Water Reminder',   desc: 'Reminds you every 2 hours from 8AM to 10PM to drink water' },
    { key: 'mealReminders',    label: '🍽️ Meal Reminder',    desc: 'Fires once daily at your set time' },
    { key: 'workoutReminders', label: '🏃 Workout Reminder', desc: 'Fires once daily at your set time' },
  ];
            ...
            <div key={key} className="border-b border-sage-50 dark:border-gray-800 last:border-0 py-3">
              ...
              {reminders[key] && schedules[key] && (
                <p className="text-xs text-sage-400 dark:text-gray-500 mt-1">
                  {key === 'waterReminder'
                    ? <>⏰ Every 2 hours from <strong className="text-sage-600 dark:text-sage-400">8:00 AM</strong> to <strong className="text-sage-600 dark:text-sage-400">10:00 PM</strong></>
